...
test("migration month fan-out keeps provider scans exhaustive", async () => {
  ...
  // sweep runs twelve; it used to offer twelve and quietly search eight.
...
test("migration month summary omits fare coverage when the completed provider job is partial", async () => {
      ...
      offers: [{
        ...
        itineraries: [{
          ...
          segments: [{
            origin: "LIM",
            destination: "MIA",
            departureAt: "2026-06-16T08:00:00-05:00",
            arrivalAt: "2026-06-16T16:00:00-05:00",
          }],
        }],
  ...
  /* The month is done and it has a fare, so 06 §3 draws it «con tarifa» — there
     is no fifth state for «complete but the scan was partial». What that costs
     the month is its coverage line, which stays withheld above rather than
     guessed, and the sweep still reports itself partial below. Keying the state
     on `searchMeta.partial` instead left the «Act.» spinner running for good on
     a month that had stopped. */
