...
Do not copy the Chrome profile, cookie database, session SQLite database, or `CBPLUS_TOKEN` from another host. These artifacts are ephemeral state, may contain tokens or purchase paths, and do not prove that authentication is reproducible on the production host.
...
- Do not copy `/etc/fly-desk.env`, `/var/lib/fly-desk`, the `fly-desk-chrome.service` profile, or files under `/tmp` from another host.
...
- `/etc/fly-desk.env` was reconstructed from the canonical secret store, not copied from another host;
