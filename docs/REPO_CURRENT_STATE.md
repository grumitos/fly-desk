...
- the list's scrollbar is drawn (`frontend/src/components/results/ResultsScrollbar.tsx`), the browser's having been off since `.fd-list-viewport` took `scrollbar-width: none`. It reads `scrollHeight` / `clientHeight` / `scrollTop` off the viewport through a `ResizeObserver` and the scroll event — never a row count, which a list of mixed group and plain cards and a partial-search skeleton would misreport — so the thumb measures the list that is built, and shrinks as the window appends batches. It is draggable, `aria-hidden` and outside the tab order, and it stands down under `@container fdshell (max-width: 719.98px)` where the gesture is the convention. It has a channel of its own — 10 px of pointer target around 6 px of drawing, reserved on `.fd-list-body` so the column header reserves it with the rows — and the 10 is paid for out of the row's own `padding-right`, so every lane stays at the pixel it was at and no threshold moves. Drawn over the rows it took 18 px of every row's hit area, which on a list of clickable fares is a defect and not an overlay bar's usual licence. Its sheet is deliberately **unlayered**: `index.css` leaves `@layer` behind partway through and outranks everything layered after that, so a rule of this kind inside `@layer utilities` would be inert without saying so
...
- `patches/bun-plugin-tailwind@0.1.2.patch` removes the plugin's `bun >= 1.0.0` peer, which used to pull the npm `bun` package with ~350 MB of platform binaries into every release; the runtime is the system Bun and the lockfile no longer resolves that package
