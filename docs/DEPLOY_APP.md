...
- Busqueda/cache: `SEARCH_MAX_FUTURE_DAYS`, `SEARCH_REVALIDATION_CACHE_TTL_MS`, `SEARCH_COMPLETED_SESSION_TTL_MS`, `SEARCH_COMPLETED_SESSION_RESIDENT_BUDGET_BYTES`, `FLY_DESK_QUOTATION_RATE_TIMEOUT_MS`, `FLY_DESK_SEARCH_WORKER_PROCESSES`, `FLY_DESK_MIGRATION_CONCURRENT_MONTHS`, `FLY_DESK_APP_DATA_DIR`, `FLY_DESK_SESSION_DB_PATH`, `FLY_DESK_LOCATION_SUGGESTION_DB_PATH`, `FLY_DESK_LOCATION_USAGE_DB_PATH`, `FLY_DESK_QUOTATION_RATE_CACHE_PATH`. Si `FLY_DESK_APP_DATA_DIR` esta definido, los SQLite de sesion, autocomplete y ranking global usan ese directorio salvo override `*_DB_PATH` especifico. El presupuesto residente default es 128 MiB y no elimina filas ni redirects antes del TTL; la consulta externa de tasa tiene un timeout default de 1500 ms (maximo 10000 ms).
