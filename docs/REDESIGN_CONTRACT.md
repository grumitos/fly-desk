...
| — (the plates draw the row once, at 1440, with one elastic lane) | Three of the six lanes grow with the desk, each to the widest string it can be handed, and what none of them can use is split as spacing between the columns | **The plate is under-specified here, so it could not be copied.** `Main.dc.html` is drawn at 1440 and only at 1440, where the row has 37px of slack and the single elastic lane hides it. Measured on the deployed application, the same structure at 1600, 1920 and 2560 put 197, 357 and 357 px into that one lane — the stops label — which measures 75: at 1920 the row drew 418px of content, then **357px of nothing**, then the baggage, the price and the provider. The slack is `list − 787`, 787 being every lane at its floor, and three lanes can use it, in the order in which their own data is being cut: the stops lane first, +68 (75 → 143, «9 escalas · MMM, MMM +7» at 142.63 being the widest string the model can emit), because it is the only lane that starts below what its data needs and because `DETAIL_COLUMN_MIN_LIST_PX` is 787 + 37 exactly to take it from «1 escala · BOG» to «2 escalas · BOG, PTY» — which is only true if it is first; then «who flies», +28 (142 → 170), whose second line is cut on every codeshare («Operado por Aerolíneas Argentinas» measures 169.94 at the 10/600 it is painted in, the prefix being drawn by the sheet); then the price, +19 (116 → 135, «USD 123,456.78» at 134.41, the widest total the nine passengers `MAX_SEARCH_PASSENGERS` allows can be quoted), last because its figure is `nowrap` with no ellipsis and bleeds into the gap rather than losing digits. Nothing else grows: the marks are artwork, the pair is 32 of pictogram in 36, and the leg's three fixed lanes are already measured to their worst case. That spends 115 of slack, and the widest list this application draws is 1144, whose slack is 357 — so the remaining 242 is spent as four equal shares of spacing, one per boundary between two **named** columns, carried by the lane whose content aligns away from it. Four and not five: the header leaves the mark's lane unnamed («a mark is not a column of values»), so the 12 between a carrier's mark and its name is not a column gap and does not grow. A 1440 desk is untouched to the byte — same tracks, same 428, same 112 in the stops lane — which is the constraint the whole derivation is bounded by. Pinned by «the row's slack goes to the lanes whose data can use it», which measures the three ceiling strings with a `Range` in the row itself rather than trusting the numbers written here. |
...
| — | The order is the column header, and it has four radios | «Ordenar · Precio ｜ Duración» was a control of its own for something a column header already does, in the place everybody looks for it. `.fd-card--head` is the `radiogroup` now, with the same accessible names the segmented had, so what changed is the shape and not the semantics — the radios arrive in column order rather than in the segmented's, which is the only thing the cases had to renegotiate. **Owner-decided** when the backend's catalogue grew to four: «Salida» and «Escalas» arrived as two more options of a segmented control that no longer existed, and they did not need controls of their own — «Horario» and «Escalas» were already columns of this header, and making a column sort is what the header of a table is for. So the group is **Horario ｜ Duración ｜ Escalas ｜ Precio** over the lanes that already name that data, and the lanes that cannot be ordered by (Aerolínea, Tramo, Eq., Prov.) stay labels: the backend has four criteria and the group offers exactly those four. The active arrow costs its cell 15px and one lane cannot pay it — «Duración» measures 56.89 in a lane of 66 — so that cell hangs 5.89 into the 12px gap beside it rather than the lane growing, because the lane is `RESULT_LEG_FIXED_PX` and widening it moves 787 and 824 (see below). The phone keeps `.fd-result-sort-compact`, which has no column header to put the order in and cycles the four instead of swapping two. |
...
rather than nudged. And the rule has a second half, learned the hard way: the
stacked row's lanes also answer to the **mode**, so no mode may ask for more
phone than another to keep the code, and the width every mode has to hold it at
is 360. `test/ui/results.playwright.ts` pins both sweeps — thirteen desk widths
in «the card keeps a lane for the airport codes», and four `searchMode`s at 360,
375 and 390 in «the stops label keeps its airport code in every mode» — so at
every width and in every mode the lane holds the label it is showing, the legs
track never overflows, and the disposition answers the list width and nothing
else.
