...
- El servidor web limita el body entrante a 1 MiB y lo materializa una sola vez para reconstruir la solicitud confiable; la delegacion al runner reutiliza ese stream sin crear una segunda copia y mantiene un timeout bounded durante la respuesta. No bajar `FLY_DESK_SEARCH_SERVICE_TIMEOUT_MS` por debajo del default operativo.
