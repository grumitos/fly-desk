...
export function providerStatusTtlMsFor(prewarmIntervalMs: number): number {
  if (!Number.isFinite(prewarmIntervalMs) || prewarmIntervalMs <= 0) {
    return DEFAULT_PROVIDER_STATUS_TTL_MS;
  }

  return Math.max(DEFAULT_PROVIDER_STATUS_TTL_MS, prewarmIntervalMs * 2 + 60_000);
}
...
export class ProviderStatusTracker {
  ...
  /** What `/api/provider-status` has to report as `staleAfterMs`. */
  get ttlMs(): number {
    return this.#ttlMs;
  }
