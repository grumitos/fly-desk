...
export function resolveListSearchResultLimit(request: SearchRequest): number {
  if (typeof request.filters.maxResults === "number" && request.filters.maxResults > 0) {
    return Math.max(1, Math.trunc(request.filters.maxResults));
  }

  return LIST_SEARCH_RESULT_LIMIT;
}
