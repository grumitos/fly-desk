...
export function AllSchedulesPanel({
  ...
  return (
    <div
      ...
      className="fd-motion-emergente absolute inset-x-0 top-full z-30 mt-1.5 max-h-[19rem] overflow-hidden rounded-xl border border-border bg-popover text-popover-foreground shadow-[0_14px_40px_color-mix(in_srgb,var(--color-foreground)_15%,transparent)]"
    >
              ...
              <span className="grid gap-1">
                {model.legs.map((leg) => (
                  <span key={leg.label} className="fd-schedule-row__leg">
                    ...
                    <span className={cn("fd-card__leg-stops", `is-${leg.stopsTone}`)}>{leg.stopsLabel}</span>
                  </span>
                ))}
              </span>
