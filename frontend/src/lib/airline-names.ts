...
function isLatamVariant(value: string): boolean {
  ...
  const base = words[0]
  if (base !== "LATAM" && base !== "LAN" && base !== "TAM") return false
...
function applyKnownAirlineStyle(value: string): string {
  ...
  if (codeDisplayName) return codeDisplayName

  if (isLatamVariant(value)) return "LATAM"
...
export function resolveAirlineDisplayName(input: {
  ...
  for (const normalized of codeDisplayNames) return normalized
